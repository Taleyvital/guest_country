...
import { countLetterOccurrences, findNextPlayer, checkGameEnd } from '../utils/game';
...
export default function CountryGuessGame() {
  ...
  // Référence pour aiTurn pour éviter la dépendance circulaire
  const aiTurnRef = useRef<(() => void) | null>(null);

  // Déclarer nextTurn avec useCallback pour qu'il soit stable
  const nextTurn = useCallback(async () => {
    ...
    if (players[next] && players[next].skipNextTurn) {
      ...
      // Répéter pour le prochain joueur
    ...
    // Si c'est le tour de l'IA, la laisser jouer automatiquement
    if (gameMode === 'solo' && next === 1 && !players[next].eliminated && players[next].isAI) {
      setTimeout(() => {
        aiTurnRef.current?.();
      }, 1000);
    }
  }, [gameMode, currentPlayer, players, setPlayers, setCurrentPlayer, setMessage, updateOnlineGame]); // Dépendances

  // useAiLogic maintenant peut utiliser nextTurn
  ...
  // En mode solo, c'est mon tour seulement si c'est le joueur humain (id 0) ET que ce n'est pas l'IA
  const isMyTurn = gameMode === 'online' 
    ? currentPlayerData?.id === myPlayerId 
    : gameMode === 'solo' 
    ? currentPlayer === 0 && !currentPlayerData?.isAI
    : true;
